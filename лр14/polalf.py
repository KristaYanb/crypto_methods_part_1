...
def index_similar(stroka):
    ...
    key = search_key_ind(alf, freq, text_only, len_key)
...
def autocor(stroka):
    ...
    plot_autocor(arr_gamma,x)
    #key = search_key_autocor(alf, freq, text_only, len_key)
    key = search_key_ind(alf, freq, text_only, len_key)
    return len_key, key
